...
export default function BooksByCategory() {
  ...
  const fetchCategoryAndBooks = async () => {
    try {
      ...
      // جلب كتب التصنيف
      const booksRes = await api.get(`/Books?categoryId=${id}&pageSize=100`);
      const booksData = booksRes.data?.data?.items || booksRes.data?.data || booksRes.data || [];
      setBooks(Array.isArray(booksData) ? booksData.filter(b => b.isPublished) : []);
  ...
  const handleDownload = async (bookId) => {
    ...
    try {
      ...
      const url = book?.pdfPath || book?.pdfUrl;
      if (!url) {
        alert('PDF not found');
        return;
      }
  ...
  const filtered = books.filter(book => {
    ...
    const matchesLevel = !level || book.academicLevel === level;
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          >
            <button
              ...
              className="inline-flex items-center gap-2 text-white/70 hover:text-white mb-6 transition group"
            >
              <ArrowLeft className="w-4 h-4 group-hover:-translate-x-1 transition" />
              Back to Categories
            </button>
            
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4" style={{ fontFamily: "'Playfair Display', 'Cinzel', serif" }}>
              <span className="text-white">{category?.nameEn || 'Category'}</span>
              <span className="bg-gradient-to-r from-[#c9a96e] to-[#e4c088] bg-clip-text text-transparent"> Books</span>
            </h1>
            
            <p className="text-lg text-blue-100/60 max-w-2xl mx-auto">
              {category?.description || 'Browse books in this category'}
            </p>
          </motion.div>
        </div>
